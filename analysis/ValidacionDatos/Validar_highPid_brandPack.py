...
# Leemos el path
df_products = spark.read.format("parquet").load(
    "/mnt/consumezone/Argentina/Datascience/SkuPrioritization/promotor/Features/products"
)
...
df_products_t = df_products_t.select(
    "SKU_ID", "HIGH_pid", "product_desc", "brand_family", "brand_pack"
).orderBy("brand_family")
...
def SendEmail(sender, recipient, subject, message, csv):
    ...
    archivo_csv.add_header(
        "content-disposition",
        "attachment",
        filename="Productos con brand-pack distintos.csv",
    )
