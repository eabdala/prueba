...
duplicates_productSlim = (
    df_prodslim.groupBy("pid").count().alias("count").filter(col("count") > 1)
)
...
def SendEmail(recipient, subject, message, attached_1):
    ...
    file_attached_1.add_header(
        "content-disposition", "attachment", filename="Duplicados_productSlim.csv"
    )
