...
df_upsellModel = spark.read.format("parquet").load(
    "/mnt/consumezoneprod/Argentina/Commercial/Arena/Upsellmodel"
)
...
df_upsellModel = df_upsellModel.filter(df_upsellModel.row_number == 1).select(
    "poc_code", "HIGH_pid", "rating", "recommendation_ts", "LAST_UPDATE"
)
...
df_upsellModel.write.mode("overwrite").partitionBy("LAST_UPDATE").format(
    "parquet"
).save(
    "/mnt/consumezone/Argentina/Datascience/SkuPrioritization/Score/min_rating_per_poc"
)
