...
cant_filas_lastDay = df_upsell_cz_prod.filter(
    to_date(col("LAST_UPDATE")) == current_date
).count()

# Cant. de poc_id distintos filtrando la columna recommendation_ts por el dia anterior
cant_pocid_lastDay = (
    df_upsell_cz_prod.filter(to_date(col("LAST_UPDATE")) == current_date)
    .select(countDistinct("poc_code"))
    .collect()[0][0]
)
...
def SendEmail(recipient, subject, message):
    ...
    )  # check server and port with your provider
    server.ehlo()
    server.starttls()
...
if cant_filas_lastDay == 0:
    asunto = "Arena UpsellModel - No se encontraron datos para la ultima fecha " + str(
        yesterday_date
    )
    destinatario = (
        "mmarzora@quilmes.com.ar, mamalnic@quilmes.com.ar, mibassan@quilmes.com.ar"
    )
    mensaje = (
        "Fecha: "
        + str(yesterday_date)
        + "<br> Cantidad filas: "
        + str(cant_filas_lastDay)
        + "<br> Cantidad poc_id distintos: "
        + str(cant_pocid_lastDay)
    )
